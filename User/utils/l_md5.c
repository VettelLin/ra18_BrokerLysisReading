 ...
 */
void init_md5(md5_context_st *context)
{
 ...
 */
static void transform_md5_block(unsigned int state[4], unsigned char block[64])
{
...
/*
    MD5 block update_md5_context operation. Continues an MD5 message-digest operation, processing another message block,
    and updating the context.
 */
void update_md5_context(md5_context_st *context, unsigned char *input, unsigned int inputLen)
{
    ...
    if (inputLen >= partLen) {
        ...
        transform_md5_block(context->state, context->buffer);
        for (i = partLen; i + 63 < inputLen; i += 64) {
            transform_md5_block(context->state, &input[i]);
        }
        index = 0;
    } else {
        i = 0;
 ...
 */
void finalize_md5(unsigned char digest[16], md5_context_st *context)
{
    ...
    update_md5_context(context, PADDING, padLen);

    /* Append length (before padding) */
    update_md5_context(context, bits, 8);
...
int calc_md5(char *digest, unsigned char *buf, unsigned int length)
{
	...
	init_md5(&context);
	update_md5_context(&context, (unsigned char*) buf, (unsigned int) length);
	finalize_md5(digest, &context);

    // memcpy(digest, hash, CRYPT_HASH_SIZE);

    return CRYPT_HASH_SIZE;
}
...
int calc_md5hex(char *digest, unsigned char *buf, unsigned int length)
{
	...
	len = calc_md5(hash, buf, length);

    if(len <= 0){
        return len;
    }

	for (i = 0, r = digest; i < len; i++) {
	    *r++ = hex[hash[i] >> 4];
	    *r++ = hex[hash[i] & 0xF];
	}
...
char *get_md5(unsigned char *strbuff, unsigned char *data, unsigned int length, unsigned char *prefix)
{
    ...
    if(calc_md5hex(result, data, length) <= 0){
        return NULL;
    }
	...
	if (strbuff) {
	    if (prefix) {
	    	len = strlen(prefix);
	        strcpy(strbuff, prefix);
	    }
	    strcpy(strbuff + len, result);
	}
...
{
	char digest[MD5_HASH_HEXSIZE + 4];

	if(calc_md5hex(digest, data, len) <= 0){
		return -1;
	}

	// printf("calculate data md5 : digest = %s\n", digest);

	if(memcmp(digest, md5_digest, MD5_HASH_HEXSIZE)){
		return 0;
	}else{
		return 1;
	}
